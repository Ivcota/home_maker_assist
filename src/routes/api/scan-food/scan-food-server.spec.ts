...
vi.mock('$lib/infrastructure/ai-food-photo-scanner.js', async () => {
	...
	return {
		AIFoodPhotoScanner: scanner,
		AIFoodPhotoScannerLive: Layer.succeed(ReceiptScanner, scanner)
	};
});
...
const makeRequest = () => {
	...
	formData.append(
		'image',
		new File([new Uint8Array([1, 2, 3])], 'food.jpg', { type: 'image/jpeg' })
	);
	return new Request('http://localhost/api/scan-food', { method: 'POST', body: formData });
};

const fakeUser = { id: 'user-1', name: 'Test', email: 'test@example.com' };

describe('POST /api/scan-food', () => {
