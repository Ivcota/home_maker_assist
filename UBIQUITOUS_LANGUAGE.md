...
- **"Item"** is used loosely throughout the codebase to mean **Food Item**, **Restock Item**, **Extracted Food Item**, or even a generic list entry. Always qualify: a *Food Item* is in inventory, a *Restock Item* is on the restock list, an *Extracted Food Item* is fresh from scanning.
- **"Amount" vs "Quantity"** — these are *not* interchangeable. **Amount** is a percentage (0–100) for bulk items tracked by `amount` type. **Quantity** is a whole-number count for discrete items tracked by `count` type. A Food Item has one or the other, never both.
