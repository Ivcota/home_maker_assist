	...
	<div class="fixed bottom-20 left-1/2 z-50 flex -translate-x-1/2 flex-col gap-2">
		{#each toasts as toast (toast.id)}
			...
			>
				...
				{#if toast.undoRecipe}
					<form method="POST" action="?/restore" use:enhance={() => () => dismissToast(toast.id)}>
						<input type="hidden" name="id" value={toast.undoRecipe.id} />
						<input
							type="hidden"
							name="trashedAt"
							value={toast.undoRecipe.trashedAt?.toISOString()}
						/>
...
<main class="mx-auto w-[85%] max-w-5xl pt-6 pb-24">
	...
	{#if scanning && scanProgress}
		<div class="mb-4 rounded-xl border border-[#e8e2d9] bg-white px-4 py-3">
			<p class="mb-2 text-sm text-[#5c4a2a]">
				Processing {scanProgress.current} of {scanProgress.total} photo{scanProgress.total === 1
					? ''
					: 's'}…
			</p>
			...
			{#each batchRecipes as batchRecipe (batchRecipe.batchId)}
				...
				<div class="rounded-2xl border border-[#e8e2d9] bg-white p-5 shadow-sm">
					...
					>
						<div class="mb-4">
							<label
								class="mb-1 block text-xs font-semibold tracking-wide text-[#8a7a6a] uppercase"
								for="batch-name-{batchRecipe.batchId}"
							>
								Recipe Name
							</label>
							<input
						...
						{#if duplicateName}
							<div
								class="mb-4 rounded-xl border border-yellow-200 bg-yellow-50 px-3 py-2 text-sm text-yellow-800"
							>
								A recipe named <strong>"{duplicateName}"</strong> already exists. You can save anyway
								or discard this one.
							</div>
						{/if}
	...
	{:else}
		...
		<div class="mb-4 flex gap-1 overflow-x-auto rounded-xl bg-[#f0ece6] p-1">
			{#each [{ id: 'all' as TabId, label: 'All' }, { id: 'ready' as TabId, label: 'Ready' }, { id: 'almost-ready' as TabId, label: 'Almost Ready' }, { id: 'need-to-shop' as TabId, label: 'Need to Shop' }, { id: 'trash' as TabId, label: 'Trash' }] as tab}
				<button
					onclick={() => {
						activeTab = tab.id;
					}}
					class="flex-shrink-0 rounded-lg px-3 py-1.5 text-sm font-semibold transition {activeTab ===
					tab.id
						? 'bg-white text-[#2c2416] shadow-sm'
						: 'text-[#8a7a6a] hover:text-[#2c2416]'}"
				>
					{tab.label}
				</button>
			{/each}
						...
						<div class="rounded-2xl border border-[#e8e2d9] bg-white shadow-sm">
							<div class="px-5 py-4">
								<div class="flex items-start justify-between gap-2">
									...
									>
										<h2
											class="font-[Cormorant_Garamond,serif] text-lg font-semibold text-[#2c2416]"
										>
											{recipe.name}
										</h2>
										{#if recipe.ingredients.length > 0}
											<p class="mt-0.5 text-sm {readinessColor[readiness.status]}">
												{readiness.matched} of {readiness.total} ingredient{readiness.total === 1
													? ''
													: 's'} · {readinessLabel[readiness.status]}
											</p>
										{:else}
							...
							{#if isExpanded && recipe.ingredients.length > 0}
								<div class="border-t border-[#f0ece6] px-5 pt-3 pb-4">
									<ul class="flex flex-col gap-1.5">
										{#each matches as match}
											<li class="flex items-center gap-2 text-sm">
												...
												{:else}
													<svg
														class="h-4 w-4 flex-shrink-0 text-[#c0a880]"
														viewBox="0 0 24 24"
														fill="none"
														stroke="currentColor"
														stroke-width="2"
				...
				<div class="flex flex-col gap-3">
					{#each trashedRecipes as recipe (recipe.id)}
						...
						>
							<div class="flex items-start justify-between gap-2">
								<h2
									class="font-[Cormorant_Garamond,serif] text-lg font-semibold text-[#2c2416] line-through"
								>
									{recipe.name}
								</h2>
...
{#snippet ingredientEditor(
	...
	<div class="mb-4">
		<div class="mb-2 flex items-center justify-between">
			<span class="text-xs font-semibold tracking-wide text-[#8a7a6a] uppercase">Ingredients</span>
			<button
				type="button"
				onclick={onAdd}
				class="text-xs font-semibold text-[#5c4a2a] hover:underline"
