...
import { UnreadableImageError, NoItemsExtractedError, AIProviderError } from './errors.js';
...
describe('extractItemsFromReceipt', () => {
	it('returns items with names normalized (trimmed and title-cased)', async () => {
		const items = [makeItem({ name: '  whole milk  ' }), makeItem({ name: 'CHICKEN BREAST' })];
	...
	it('propagates AIProviderError from scanner', async () => {
		const result = await Effect.runPromise(
			extractItemsFromReceipt(TEST_INPUT).pipe(
				Effect.provide(
					makeFakeReceiptScanner(Effect.fail(new AIProviderError({ cause: 'timeout' })))
				),
				Effect.flip
			)
		);
